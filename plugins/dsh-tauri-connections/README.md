...
原有工具栏保持样式。新会话直接调用当前连接的原生新建操作，沿用当前会话的工作区；没有当前会话时使用该 DSH 最近的工作区，没有工作区则进入原生空白页。不弹连接或工作区选择窗口，按钮提示显示当前连接名。工作区行的悬停按钮和右键菜单可直接新建会话；连接行同样提供新建工作区，只需填写或浏览目录。原有重命名、删除工作区、分叉和归档会话操作保留，各操作按连接和 ID 执行。

单列表合并全部已连接 DSH 的会话，不保留连接或工作区分组；每行右侧显示连接和工作区按钮，点击分别提供新建工作区和新建会话。排序应用于整个列表。正在运行显示小型旋转标记，结束且未查看显示小点，打开后清除；后台连接当前会话的完成提示也会保留。状态来自客户端运行数据及其运行结束事件，不按更新时间猜测完成。搜索覆盖全部连接，内容索引不可用时保留标题匹配并提示。点击任何会话即可进入其所属连接，不需先点击连接标题。
