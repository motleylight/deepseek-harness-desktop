...
import type { DirectoryListing, SearchResults, WorkspaceRequest } from './workspace-client'
...
import { parseDirectory, parseSearch } from './workspace-client'

export type WorkspaceAction = 'add-workspace' | 'search'
interface Props {
...
/** Directory registration stays on its invoking connection; search covers connected hosts. */
          ...
          <Modal.Dialog>
            ...
            <If cond={action !== 'search'}>
              <Modal.Footer>
                ...
                <Button onPress={submit} isPending={busy} isDisabled={!connection || loading || !path.trim()}>{t(`workspace.${action}`)}</Button>
              </Modal.Footer>
            </If>
          </Modal.Dialog>
        </Modal.Container>
